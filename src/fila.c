...
void inserir (fila_ordenada_t * fila, aviao_t * dado) {
    ...
    {   
        ...
        if (dado->combustivel > 1)
        {
            ...
            printf("o aviao %d esta com pouco combustivel\n",dado->id);
            if (fila->primeiro->anterior == NULL)
            {
                adicionado->proximo = fila->ultimo;
                fila->ultimo->anterior = adicionado;
                fila->ultimo = adicionado;
                ...
                adicionado->proximo = fila->primeiro;
                adicionado->anterior = fila->primeiro->anterior;
                fila->primeiro->anterior = adicionado;
                fila->primeiro->anterior->proximo = adicionado;
            }
        }
        fila->n_elementos++;
    }
